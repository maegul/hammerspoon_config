...
local function cacheWindow(win, move_to)
   local win = win or hs.window.focusedWindow()
   -- if (not win) or (not win:id()) then return end -- Errol commented and modified below
...
function obj.center(win)
   -- print(win) -- why is win a table here? it's the whole obj or spoon?
   -- for k,v in pairs(win) do
   --    print(k, v)
   -- end
   -- local win = win or hs.window.focusedWindow()
   local win = hs.window.focusedWindow() -- Necessary to fix this bug
   if win then
      -- print('inside center', win)
      -- print(win:id())
